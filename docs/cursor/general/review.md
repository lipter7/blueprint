...
TypeMeaningExample**Added lines**New code additions`+ const newVariable = 'hello';`**Deleted lines**Code removals`- const oldVariable = 'goodbye';`**Context lines**Unchanged surrounding code` function example() `
## Agent Review
...
SettingDescription**Auto-run on commit**Scan your code for bugs automatically after each commit**Include submodules**Include changes from Git submodules in the review**Include untracked files**Include untracked files (not yet added to Git) in the review
## Review UI
