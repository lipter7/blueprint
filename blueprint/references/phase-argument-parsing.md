...
```bash
PHASE_INFO=$(node ~/.claude/blueprint/bin/blueprint-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/blueprint/bin/blueprint-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/blueprint/bin/blueprint-tools.js find-phase "${PHASE}" --raw)
```
