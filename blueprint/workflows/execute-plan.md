...
@~/.claude/blueprint/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/blueprint/bin/blueprint-tools.js init execute-phase "${PHASE}" --include state,config)
```
...
If `.blueprint/` missing: error.
</step>

<step name="identify_plan">
```bash
# Use plans/summaries from INIT JSON, or list files
ls .blueprint/phases/XX-name/*-PLAN.md 2>/dev/null | sort
ls .blueprint/phases/XX-name/*-SUMMARY.md 2>/dev/null | sort
```
...
```bash
grep -n "type=\"checkpoint" .blueprint/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="bp-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
if [ ! -f .blueprint/agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > .blueprint/agent-history.json
fi
rm -f .blueprint/current-agent-id.txt
if [ -f .blueprint/current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat .blueprint/current-agent-id.txt)
...
2. Per segment:
   - Subagent route: spawn bp-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
cat .blueprint/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
```bash
ls .blueprint/phases/*/SUMMARY.md 2>/dev/null | sort -r | head -2 | tail -1
```
...
See `~/.claude/blueprint/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/blueprint/references/checkpoints.md for details.
</step>
...
```bash
grep -A 50 "^user_setup:" .blueprint/phases/XX-name/{phase}-{plan}-PLAN.md | head -50
```

If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/blueprint/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.blueprint/phases/XX-name/`. Use `~/.claude/blueprint/templates/summary.md`.
...
Update STATE.md using blueprint-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ~/.claude/blueprint/bin/blueprint-tools.js state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/blueprint/bin/blueprint-tools.js state update-progress

# Record execution metrics
node ~/.claude/blueprint/bin/blueprint-tools.js state record-metric \
...
node ~/.claude/blueprint/bin/blueprint-tools.js state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.claude/blueprint/bin/blueprint-tools.js state add-blocker "Blocker description"
```
</step>

<step name="update_session_continuity">
Update session info using blueprint-tools:

```bash
node ~/.claude/blueprint/bin/blueprint-tools.js state record-session \
...
```bash
node ~/.claude/blueprint/bin/blueprint-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .blueprint/phases/XX-name/{phase}-{plan}-SUMMARY.md .blueprint/STATE.md .blueprint/ROADMAP.md
```
</step>

<step name="update_codebase_map">
If .blueprint/codebase/ doesn't exist: skip.
...
```bash
node ~/.claude/blueprint/bin/blueprint-tools.js commit "" --files .blueprint/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/bp:execute-phase {phase}` + `/bp:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/bp:plan-phase {Z+1}` + `/bp:verify-work {Z}` + `/bp:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/bp:complete-milestone` + `/bp:verify-work` + `/bp:add-phase` |
