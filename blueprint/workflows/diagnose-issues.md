...
Debug files use the `.blueprint/debug/` path (hidden directory with leading dot).
</paths>
...
- truth: "Comment appears immediately after submission"
  ...
  debug_session: .blueprint/debug/comment-not-refreshing.md
```
...
```bash
node ~/.claude/blueprint/bin/blueprint-tools.js commit "docs({phase}): add root causes from diagnosis" --files ".blueprint/phases/XX-name/{phase}-UAT.md"
```
</step>
