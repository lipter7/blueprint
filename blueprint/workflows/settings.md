...
AskUserQuestion([
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (bp/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (bp/{version}-{name})" }
    ]
  }
])
```
